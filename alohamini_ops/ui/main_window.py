...
QPushButton#cameraControl { min-height: 22px; max-height: 30px; padding: 3px 7px; }
...
QLineEdit { background: #14181c; border: 1px solid #3a424b; border-radius: 5px; padding: 6px; min-height: 28px; color: #e7edf2; }
...
QLabel#summaryTile { background: #151a1f; border: 1px solid #303943; border-radius: 5px; padding: 8px; min-height: 52px; }
...
class MainWindow(QMainWindow):
    def __init__(self, context: AppContext):
        ...
        self.ui.log_panel.append("INFO", "GUI 已就绪。")
    ...
    def _connect_signals(self) -> None:
        ...
        c.open_ssh.clicked.connect(self.open_ssh_session)
        c.check_ssh.clicked.connect(
            lambda: self.run_command("ssh_session_check", self.context.scripts.script("ssh_session.sh") + ["check"])
        )
        c.close_ssh.clicked.connect(
            lambda: self.run_command("ssh_session_close", self.context.scripts.script("ssh_session.sh") + ["close"])
        )
        ...
        cal.calibrate.clicked.connect(
            lambda: self.open_calibration_terminal(
                "calibrate_leaders.sh",
                next_step="按终端提示移动 Leader，完成后刷新状态。",
            )
        )
        cal.use_existing.clicked.connect(
            lambda: self.open_calibration_terminal(
                "use_leader_calibration.sh",
                next_step="终端完成后刷新状态，确认 Leader 校准文件可用。",
            )
        )
        cal.scan_left_leader.clicked.connect(lambda: self.scan_leader_mapping_serial("left"))
        cal.scan_right_leader.clicked.connect(lambda: self.scan_leader_mapping_serial("right"))
        cal.apply_leader_mapping.clicked.connect(self.apply_leader_port_mapping)
        cal.verify_leader_mapping.clicked.connect(self.verify_leader_port_mapping)
        cal.check_pi_followers.clicked.connect(
            lambda: self.open_calibration_terminal(
                "check_pi_servos.sh",
                next_step="查看终端中的 follower 映射和舵机 ID，再执行左右 Follower 校准。",
            )
        )
        cal.calibrate_left_follower.clicked.connect(lambda: self.open_follower_calibration_terminal("left"))
        cal.calibrate_right_follower.clicked.connect(lambda: self.open_follower_calibration_terminal("right"))
        ...
        diag.local_log.clicked.connect(
            lambda: self.run_command(
                "local_teleop_log",
                [
                    "bash",
                    "-lc",
                    f"tail -120 {shlex.quote(str(self.context.config.local_teleop_log))} 2>/dev/null || true",
                ],
            )
        )
    ...
    @Slot()
    def open_ssh_session(self) -> None:
        if self._busy_with_robot():
            self.ui.log_panel.append("WARN", "机器人任务运行中，暂不切换 SSH 会话。")
            return
        target = self._read_pi_target_inputs()
        if target is None:
            return
        self.context.set_pi_target(*target)
        self.ui.connection_tab.ssh_state.setText("SSH 会话: 等待终端认证")
        self.open_terminal("ssh_session.sh", ["open"])
    ...
    def handle_command_result(self, label: str, code: int, output: str) -> None:
        if code == 255:
            self.ui.connection_tab.ssh_state.setText("SSH 会话: 认证失败")
            self.ui.link_state.setText("网络链路\n需要 SSH 认证")
            self.ui.log_panel.append(
                "WARN",
                "SSH 返回 255。请检查 Pi 地址/用户名；无密钥调试时先建立临时 SSH 会话。",
            )
        if label == "ssh_session_check":
            self.ui.connection_tab.ssh_state.setText("SSH 会话: 可用" if code == 0 else "SSH 会话: 未建立")
            self.ui.link_state.setText("网络链路\nSSH 可用" if code == 0 else "网络链路\n需要 SSH 认证")
        elif label == "ssh_session_close":
            self.ui.connection_tab.ssh_state.setText("SSH 会话: 已断开")
    ...
    def _update_summary_from_command(self, label: str, code: int, output: str) -> None:
        ...
        if label == "stop_pi_host":
            ...
            return
        if label == "status":
            host_ok = "alohamini_host" in output
            ssh_ok = code == 0
    ...
    def open_terminal(self, script_name: str, extra_args: list[str] | None = None) -> None:
        try:
            subprocess.Popen(
                self.context.scripts.open_terminal_command(script_name, extra_args),
                env=self.context.config.env,
            )
            self.ui.log_panel.append("INFO", f"已打开终端: {script_name}")
        except Exception as exc:
            self.ui.log_panel.append("ERROR", f"打开终端失败: {exc}")

    def open_calibration_terminal(self, script_name: str, extra_args: list[str] | None = None, next_step: str | None = None) -> None:
        if next_step:
            self.ui.calibration_tab.set_next_step(next_step)
        self.open_terminal(script_name, extra_args)

    def open_follower_calibration_terminal(self, side: str) -> None:
        cal = self.ui.calibration_tab
        if side == "left":
            follower_id = cal.left_follower_id.text().strip()
            side_name = "左"
        else:
            follower_id = cal.right_follower_id.text().strip()
            side_name = "右"
        profile = cal.follower_profile.text().strip()
        if not follower_id:
            self.ui.log_panel.append("ERROR", f"{side_name} Follower ID 不能为空。")
            return
        if not profile:
            self.ui.log_panel.append("ERROR", "Follower arm_profile 不能为空。")
            return
        if any(ch.isspace() for ch in follower_id + profile):
            self.ui.log_panel.append("ERROR", "Follower ID 和 arm_profile 不能包含空格。")
            return
        cal.set_next_step(
            f"已打开{side_name} Follower 校准终端。按终端提示操作，结束后回到诊断页刷新状态。"
        )
        self.open_terminal("calibrate_followers_pi.sh", [side, follower_id, profile])

    def scan_leader_mapping_serial(self, side: str) -> None:
        if side == "left":
            next_step = "终端会列出当前串口。只保留左 Leader 时，把 serial 填入左侧输入框。"
        else:
            next_step = "终端会列出当前串口。只保留右 Leader 时，把 serial 填入右侧输入框。"
        self.open_calibration_terminal("leader_port_mapping.sh", ["scan"], next_step)

    def verify_leader_port_mapping(self) -> None:
        self.open_calibration_terminal(
            "leader_port_mapping.sh",
            ["verify"],
            "确认终端输出中 /dev/am_arm_leader_left/right 指向正确 Leader。",
        )

    def apply_leader_port_mapping(self) -> None:
        left_serial = self.ui.calibration_tab.left_serial.text().strip()
        right_serial = self.ui.calibration_tab.right_serial.text().strip()
        if not left_serial or not right_serial:
            self.ui.log_panel.append("ERROR", "左右 Leader serial 都不能为空。请先扫描或手动填写。")
            return
        if left_serial == right_serial:
            self.ui.log_panel.append("ERROR", "左右 Leader serial 不能相同。请重新确认物理连接。")
            return
        self.open_calibration_terminal(
            "leader_port_mapping.sh",
            ["apply", left_serial, right_serial],
            "终端完成 udev 写入后，点击“验证映射”。",
        )
    ...
    def start_record(self) -> None:
        ...
        if Path(dataset_id).is_absolute():
            ...
            return
        dataset_home_text = self.ui.dataset_tab.dataset_home.text().strip()
        if not dataset_home_text:
            self.ui.log_panel.append("ERROR", "保存根目录不能为空。")
            return
        dataset_home = Path(dataset_home_text).expanduser()
        dataset_path = dataset_home / dataset_id
        ...
        try:
            ...
            if phase_specs:
                marker_dir = dataset_home / "_phase_markers"
    ...
    def save_camera_selection(self) -> None:
        ...
        try:
            ...
            return
        self.ui.log_panel.append("INFO", f"已保存相机配置: {value}。重启 Pi Host 后生效。")
        self.ui.visual_status.setText(f"相机配置\n待重启 Host: {value}")
