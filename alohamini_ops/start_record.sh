...
GUI_RECORD_SCRIPT="${GUI_RECORD_SCRIPT:-$OPS_DIR/../compat/examples/alohamini/record_bi.py}"
if [[ ! -f "$GUI_RECORD_SCRIPT" ]]; then
  echo "GUI recording entry is missing: $GUI_RECORD_SCRIPT"
  echo "Reinstall or update the alohamini_gui companion directory."
  exit 1
fi
if ! grep -q "preview_callback" "$LOCAL_REPO/src/lerobot/scripts/lerobot_record.py" || \
   ! grep -q "frame_callback" "$LOCAL_REPO/src/lerobot/scripts/lerobot_record.py"; then
  echo "The original repository is usable from CLI, but GUI recording hooks are missing."
  echo "Run: $OPS_DIR/../scripts/patch_repo.sh --repo $LOCAL_REPO --target pc"
  exit 1
fi
